...
exports.calAll = async (req, res) => {
  ...
  try {
    for (let order of orders) {
      let datas = [];
      let costs = {};
      const preProduction = await PreProduction.findById(id).populate(
        "preOrder"
      );
      ...
      if (print_2_Data && print_2_Data?.colors?.length !== 0) {
        ...
        for (i in print_2_Data?.colors?.map((a) => a)) {
          const sendPrint = {
            ...
            floor:
              i === "0" && print_2_Data.floor_front
                ? 2
                : i === "0" && !print_2_Data.floor_front
                ? 1
                : i === "1" && print_2_Data.floor_back
                ? 2
                : i === "1" && !print_2_Data.floor_back
                ? 1
                : 1,
      ...
      if (print_4_Data && print_4_Data?.colors?.length !== 0) {
        ...
        for (i in print_4_Data?.colors?.map((a) => a)) {
          const sendPrint = {
            ...
            floor:
              i === "0" && print_4_Data.floor_front
                ? 2
                : i === "0" && !print_4_Data.floor_front
                ? 1
                : i === "1" && print_4_Data.floor_back
                ? 2
                : i === "1" && !print_4_Data.floor_back
                ? 1
                : 1,
      ...
      if (
        ...
        for (m in coatingData.methods) {
          ...
          costs[
            `coating_${coatingData.methods[m].method.type} ${coatingData.methods[m].method.subType}`
          ] = coating_cost.cost;
        }
      }
      ...
      if (
        coatingBackData &&
        coatingBackData?.methods &&
        coatingBackData?.methods?.length !== 0
      ) {
        for (m in coatingBackData.methods) {
          ...
          costs[
            `coatingBack_${coatingBackData.methods[m].method.type} ${coatingBackData.methods[m].method.subType}`
          ] = coating_cost.cost;
        }
      }
      ...
      if (
        embossData &&
        embossData?.demensions &&
        embossData?.demensions?.length !== 0
      ) {
        let emboss_blocks = [];
        let emboss_pumps = [];
        ...
        const sum_emboss_blocks =
          emboss_blocks.length > 0 ? emboss_blocks.reduce((a, b) => a + b) : 0;
      ...
      if (
        ...
        const block_k =
          blocks_k.length > 0 ? blocks_k.reduce((a, b) => a + b) : 0;
      ...
      if (glueData) {
        ...
        if (glueData.glue_dot && glueData.glue_dot.length !== 0) {
          const glueDot_cost = await calGlueDotCost(
            order,
            glueData.glue_dot.length
          );
        ...
        normal: {
          ...
          total_price:
            (
              Math.ceil(
                parseFloat(
                  (((profitNormal?.percent || 26.0) * sumCost) / 100 +
                    sumCost) /
                    order
                ) * 100
              ) / 100
          ...
          unit_price: parseFloat(
            (
              (((profitNormal?.percent || 26.0) * sumCost) / 100 + sumCost) /
              order
            ).toFixed(2)
          ),
        },
        special: {
          ...
          total_price:
            (
              Math.ceil(
                parseFloat(
                  (((profitSpecial?.percent || 24.0) * sumCost) / 100 +
                    sumCost) /
                    order
                ) * 100
              ) / 100
          ...
          unit_price: parseFloat(
            (
              (((profitSpecial?.percent || 24.0) * sumCost) / 100 + sumCost) /
              order
            ).toFixed(2)
          ),
        },
        insite: {
          ...
          total_price:
            (
              Math.ceil(
                parseFloat(
                  (((profitInsite?.percent || 17.0) * sumCost) / 100 +
                    sumCost) /
                    order
                ) * 100
              ) / 100
          ...
          unit_price: parseFloat(
            (
              (((profitInsite?.percent || 17.0) * sumCost) / 100 + sumCost) /
              order
            ).toFixed(2)
          ),
        },
...
const calRawMattCost = async (order, rawMattData) => {
  const { type, subType, gsm, width, long, cut, lay } = rawMattData;
  ...
  try {
    ...
    if (
      (rawMatt && !rawMatt.option) ||
      (rawMatt && rawMatt.option.length === 0)
    ) {
      return { data: "ไม่พบ rawMatt option", cost: 0 };
    }

    const match_option = rawMatt.option.filter(
      (item) => item.gsm === gsm && item.width === width && item.long === long
    );
    ...
    return {
      cost: calRawMaterial.paper.cost,
      data: calRawMaterial,
      amount: calRawMaterial.paper.amount,
    };
  } catch (err) {
...
const calPrint_2_Cost = async (order, print_2_Data) => {
  ...
  const { colors, lay, floor, uv, flip } = print_2_Data;
  ...
  try {
    ...
    const order_lay = flip
      ? Math.ceil(parseInt(order) / parseFloat(lay)) * 2
      : Math.ceil(parseInt(order) / parseFloat(lay));

    const option = print.option.filter(
      (item) => item.round.end >= order_lay && item.round.start < order_lay
    );
    if (option.length !== 1) {
      return { cost: 0, data: null };
    }
...
const calPrint_4_Cost = async (order, print_4_Data) => {
  ...
  const { colors, lay, floor, uv, flip } = print_4_Data;
...
const calCoatingCost = async (order, coatingData) => {
  ...
  const { method, lay, inWidth, inLong } = coatingData;
  ...
  try {
    ...
    const option =
      method.type !== "spot-uv" || method.type !== "dip-off"
        ? coating.option.filter((item) => item.subType === method.subType)
    ...
    const coating_price =
      method.type === "spot-uv" &&
      coating_option.avr * parseFloat(inWidth) * parseFloat(inLong) < 1.2
        ? 1.2
        : method.type === "dip-off"
        ? 5
        : coating_option.avr * parseFloat(inWidth) * parseFloat(inLong);
    const total_price = Math.ceil(
      parseFloat(coating_price.toFixed(2)) * order_lay
    );
    ...
    const cal_coating = {
      ...
      cost:
        total_price < coating_option.minPrice
          ? coating_option.minPrice
          : parseFloat(total_price.toFixed(2)),
      details: {
        ขนาดใบพิมพ์กว้าง: `${inWidth} นิ้ว`,
        ขนาดใบพิมพ์ยาว: `${inLong} นิ้ว`,
        ออร์เดอร์ต่อเล: order_lay,
        ประเภทเคลือบ: `${method.type} ${method.subType} Roundup(${parseFloat(
          coating_price.toFixed(2)
        )})`,
        ค่าเคลือบ:
          total_price < coating_option.minPrice
            ? coating_option.minPrice
      ...
      cal: {
        ...
        coating_price_formula:
          total_price < coating_option.minPrice
            ? `${coating_option.minPrice}`
            : `Roundup(${parseFloat(coating_price.toFixed(2))}*${order_lay})`,
        coating_price_result:
          total_price < coating_option.minPrice
            ? coating_option.minPrice
            : parseFloat(total_price.toFixed(2)),
      },
...
const calEmbossCost = async (order, embossData) => {
  const { inWidth, inLong, plateSize, mark, lay } = embossData;
  ...
  try {
    ...
    const round_option = emboss.filter(
      (item) => item.round.start < order_lay && item.round.end + 1 > order_lay
    );
    if (round_option.length === 0) {
      return { cost: 0, data: "ไม่พบ" };
    }

    const option = round_option[0].option.filter(
      (item) => item.plateSize === plateSize
    );
    ...
    const pumpPrice =
      round_option[0].round.start > 5000 &&
      round_option[0].round.end < 100000000
        ? emboss_option.pumpPrice * order_lay
        : emboss_option.pumpPrice;
...
const calHotStampCost = async (order, hotStampData) => {
  const { block, stamp } = hotStampData;
  ...
  try {
    ...
    const block_cost =
      Math.ceil(block.inWidth * block.inLong * 13 * 0.01) * 100;
    const total_block_cost = block_cost * block.lay;
    const stamp_color_cost =
      (block.inWidth * block.inLong * hotStamp.avr + 0.2) * stamp.k < 0.3
        ? 0.3
        : (block.inWidth * block.inLong * hotStamp.avr + 0.2) * stamp.k;
    const total_stamp_color_cost =
      parseFloat(stamp_color_cost.toFixed(2)) * order;
    ...
    const cal_hotStamp = {
      ...
      other_avr: 0.1,
      stamp_color_cost:
        parseFloat(stamp_color_cost.toFixed(2)) < 0.3
          ? 0.3
          : parseFloat(stamp_color_cost.toFixed(2)),
      total_stamp_color_cost: total_stamp_color_cost,
      totol_cost:
        total_stamp_color_cost + total_block_cost !== NaN
          ? parseFloat((total_stamp_color_cost + total_block_cost).toFixed(2))
          : 0,
...
const calDiecutWindowCost = async (order, diecutWindowData) => {
  const { plateSize, lay } = diecutWindowData;
  ...
  try {
    ...
    const diecut = diecuts.filter(
      (item) => item.round.start < order_lay && item.round.end + 1 > order_lay
    );

    const option = diecut[0].option.filter(
      (option) => option.plateSize === plateSize
    );
    ...
    const cal_diecut = {
      ...
      pumpPrice:
        diecut[0].round.start > 5000
          ? Math.ceil(diecut_pumpPrice * order_lay)
          : Math.ceil(diecut_pumpPrice),
      totalPrice:
        diecut[0].round.start > 5000
          ? Math.ceil(
              diecut_pumpPrice * order_lay + diecut_option.blockPrice / 2
            )
          : Math.ceil(diecut_pumpPrice + diecut_option.blockPrice / 2),
      details: {
      ...
      cal: {
        pumpPrice_formula:
          diecut[0].round.start > 5000
            ? `roundup(${diecut_pumpPrice}*${order_lay})`
            : `roundup(${diecut_pumpPrice})`,
      },
    };
...
const calDiecutCost = async (order, diecutData) => {
  const { plateSize, lay } = diecutData;
  ...
  try {
    ...
    const cal_diecut = {
      ...
      totalPrice:
        diecut[0].round.start > 5000
          ? Math.ceil(diecut_pumpPrice * order_lay + diecut_option.blockPrice)
          : Math.ceil(diecut_pumpPrice + diecut_option.blockPrice),
      details: {
        ขนาดบล๊อค: diecut_option.plateSize || plateSize,
        รอบไดคัท: diecut[0].round.join,
